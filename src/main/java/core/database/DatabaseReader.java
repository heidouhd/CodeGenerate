...
public class DatabaseReader {
     ...
     */
    public List<Table> readTables(List<String> tabs) throws Exception {
        // 获取数据库中所有的表格名
        ...
        for (String tableName : tableNames) {
            if(tabs!=null && tabs.size() != 0 && !tabs.contains(tableName.toLowerCase())){
                continue;
            }else if(!Config.CDO_TABLE_NONAME_ISCREATEALL){
                continue;
            }
            // 新建表格对象
