...
#set ($middlemanManage=$${table.name?substring(table_pre)?cap_first}Controller.view${table.name?substring(table_pre)?cap_first}($request))
<!DOCTYPE html>
<html>
<head>
    <meta charset="utf-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <title>查看${table.remark}</title>
    <meta name="description" content="查看${table.remark}">
        ...
        <ul style="list-style-type:none;width: 47%">
            <#list table.columns as column>
                <li style="display:flex; margin-bottom:10px">
                    ...
                    <input type="text" class="form-control" id="${column.name}" value="$${table.name?substring(table_pre)?cap_first}.getStringValue('${column.name}')"  disabled>
                </li>
            </#list>
        </ul>
            ...
            <div class="layui-form-item" style="text-indent: 0;margin: 0">
                <label for="inputEmail3" class="layui-form-label">
                    <button class="layui-btn" type="button"
                        ...
                        <i class="fa fa-check"></i> <a href="${table.name?substring(table_pre)?cap_first}List.htm">返回</a>
                    </button>
                </label>
            </div>
